                ...
                return (
                  ...
                  >
                    {columns.map((col) => {
                        ...
                        return (
                          ...
                          >
                            <select
                              ...
                              value={String(editValue ?? '')} // Обращаемся к значению как строке
                              onChange={(e) => {
                                setEditValue(e.target.value); // e.target.value всегда string
                              }}
                              onBlur={saveEdit}
                              onKeyDown={handleKeyDown}
                            >
                              <option value="">--</option> {/* Пустой вариант для контроля */}
                              {col.options.map((opt) => (
                                <option
                                  key={typeof opt === 'object' ? opt.value : opt}
                      ...
                      // Поддержка boolean (чекбокс)
                      if (isEditing && col.type === "boolean") {
                        return (
                          <td
                            key={col.key}
                                className="text-sm text-gray-900 border-gray-200 p-2 text-center dark:bg-gray-800 dark:text-white"
                      ...
                      return (
                        ...
                        >
                          {col.render ? (
                            col.render(row[col.key], row, extraData)
                          ) : col.type === "boolean" ? (
                            <input
                              type="checkbox"
                              checked={!!row[col.key]}
