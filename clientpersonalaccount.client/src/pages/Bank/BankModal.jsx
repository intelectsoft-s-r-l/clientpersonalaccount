...
export default function BankModal({ bank, onClose, onSave }) {
  ...
  useEffect(() => {
    ...
    const fetchAvailableBanks = async () => {
      try {
        const response = await fetch(`http://localhost:5001/api/proxy/MobileCashRegister/web/GetBanks`, {
          method: "GET",
          credentials: 'include',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const resp = await fetch(
        `http://localhost:5001/api/proxy/MobileCashRegister/web/UpsertTapxphoneSetting`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
